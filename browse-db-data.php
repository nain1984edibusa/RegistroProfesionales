...
?>
 ...
 <HTML>
<BODY>
<div style=" position:absolute;border: 0px; float:left; width:20px; height:20px; " onClick="cher.target='_self';cher.action='browse-db-data.php';cher.submit();return false;">
...
<?php
	...
	if (!$pagina) {
	   ...
	   $inicio = ($pagina - 1) * $TAMANO_PAGINA;
		...
		$sql_tot="SELECT count(idRegistroP) as tot FROM RegistroP WHERE RegistroPProfesionalID=".$_POST['us_maneja']." AND RegistroPActivo = 1 $filtro ORDER BY RegistroPCodigo ASC";
		...
		if ($inicio > $stotp['tot']){$inicio=0;};
			...
			$sql_sql="SELECT * FROM RegistroP WHERE RegistroPProfesionalID=".$_POST['us_maneja']." AND RegistroPActivo = 1 $filtro ORDER BY RegistroPCodigo ASC LIMIT ".$inicio."," . $TAMANO_PAGINA;
		$stfac = $db->query($sql_sql);
?>
<!--<?php echo $sql_sql;?>-->
<center><strong><?php echo format_cont_db('TipoProfesionalId',$_POST['us_maneja']) ?></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong>PROFESIONALES REGISTRADOS: <?php echo $stotp['tot'] ?></strong></center>
...
?>
	<tr  align='left'>
		...
		<td>
			<input type="text" name="ftitu" value="<?php if(isset($_POST['ftitu'])){echo $_POST['ftitu'];}?>" size="33" maxlength="45" onFocus="set_bgcolor(this,'<?php echo $usr_bgc?>')" onBlur="set_bgcolor(this,'')" onClick="">
		</td>		
	</tr>
	<tr><td colspan='7' align='center'><B><a class='elipse' href="Javascript:" onClick="cher.target='_self';cher.action='browse-db-data.php';cher.submit();return false">&nbsp;Filtrar&nbsp;</a>
<?php
						if($hayf){
?>
								<a class='elipse' href='JavaScript:' onClick="cher.ftelfs.value='';cher.flugar.value='';cher.ftitu.value='';cher.fmails.value='';cher.fnames.value='';cher.fcodigo.value='';cher.target='_self';cher.action='browse-db-data.php';cher.submit();return false">&nbsp;Quitar filtro&nbsp;</a> 
<?php
...
<?php
		$i=0;
			while($tit = $get_tit->fetch(PDO::FETCH_ASSOC)) {
				if ($i >0){echo '<br><br>';};
				echo mb_strtoupper($tit['FAcademicaNTitulo']);
				?>
				<!--<a title="Consultar en SENESCYT" class='info' style="font-size:11px;" href="JavaScript:" onClick="url_=url_base+'<?php echo $row['Profesional_idProfesional']?>';abrir(url_,'sene',ancho/1.5,alto,0);return false;"><?php //echo mb_strtoupper($tit['FAcademicaNTitulo']);?></a>-->
<?php				$i++;
			};
?>
			</td>
...
if ($total_paginas > 1) {
   ...
   if ($pagina != 1)
      echo '<a class="info" href="" onClick="cher.target=\'idbx'.$_POST['us_maneja'].'\';cher.action=\'browse-db-data.php\';cher.pagina.value=\''.($pagina-1).'\';cher.submit();return false"><img src="image/izq.gif" height=\'90%\' border="0"></a>&nbsp;';
      ...
      echo '<a class="info" href="" onClick="cher.target=\'idbx'.$_POST['us_maneja'].'\';cher.action=\'browse-db-data.php\';cher.pagina.value=\''.$i.'\';cher.submit();return false">&nbsp;['.$i.']&nbsp;</a>';
      }
      if ($pagina != $total_paginas)
        echo '&nbsp;<a href="" onClick="cher.target=\'idbx'.$_POST['us_maneja'].'\';cher.action=\'browse-db-data.php\';cher.pagina.value=\''.($pagina+1).'\';cher.submit();return false"><img height=\'90%\' src="image/der.gif" border="0"></a>';
   echo "</td> </tr></table> ";
};
