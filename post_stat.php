	...
	<SCRIPT LANGUAGE="JavaScript1.2" src="java/dinamico.js"></SCRIPT>
	 <SCRIPT LANGUAGE="JavaScript1.2" >
	 	...
	 	function _edit(estado){
			...
			expandit2('edit',estado)//este deberia ahbilitar el actualizar la solicitud
...
<?php
	...
	$imgsrc='';
#	$edit_soli=0;
	if ($stpos['PostulacionAsignado']==0){$imgsrc='proces1-5';/*$edit_soli=1;*/};
...
?>
<BODY onload="resiz('<?php echo $_POST['ifid']?>')">
	<form name="cher" action=" " method="post" >
		...
		<input type="hidden" name="noses" value="0">
		<input type="hidden" name="prim_upd" value="0">
		<input type="hidden" name="gen_new_post" value="<?php if (isset($_POST['gen_new_post']) and $_POST['gen_new_post']){ echo "1";}else{ echo "0";};?>"><!--para poner esta variable a uno desde el script de insercion y habilitar el boton de reportular-->
...
<!--	<b><table border='0' width='100%' align='center' style="background-attachment: fixed;background-repeat:no-repeat;" background='image/web2014.png'>
		<tr valign='top' class='menu_'>
			<td align='center'><!-- <font size='+2'><div class='cajita'> CONSULTA ESTADO SOLICITUD REGISTRO </div> </font></td>
		</tr>
	</table> </b><HR>-->
...
<!--		<td rowspan='2' class='menubar' width='7%' align="center"  onClick="post1.action='post_stat.php';post1.submit()"><a href="Javascript:"><!--<img src='image/reload.png' alt='refrescar pantalla' title='Refrescar pantalla'><br><b>Refrescar</b></a></td>-->
			<td align="center"  valign='top'>
				...
				<a href="JavaScript:" onClick="cher.target='_self';cher.action='post_stat.php';cher.submit();return false;"><img onMouseOver="swap('reloadi.png','reload.png','rld');" onMouseOut="swap('reload.png','reloadi.png','rld');" name='rld' height='20' src='image/reload.png' alt='refrescar pantalla' title='Refrescar pantalla'><!--Ayuda --></a>
			</td>
			<td rowspan='2' align="center" onMouseOver="overTD(this,'#d7ebf9');" onMouseOut="outTD(this,'');" >
				<H1> ESTADO DE LA SOLICITUD DE REGISTRO</H1>
				<table border='0' cellspacing='1' cellpadding='2' class='seccion'>
					<tr align='center'>
...
?>
					...
					<tr bgcolor='#fffff' align='center'>
						...
						<!--<td>
<?php
							if(!$stpos['PostulacionVerificado']){
								echo format_cont_db('PostulacionAsignado',$stpos['PostulacionAsignado']);
							}else{
								echo 'Validado';
							};
?>
						</td>
						<td> <?php echo format_cont_db('PostulacionVerificado',$stpos['PostulacionVerificado'])?></td>-->
<?php
...
<!--			<td class='menubar' width='7%' align="center" onClick="if(confirm('Seguro que desea terminar la sesion?')){post1.action='session.php';post1.clos.value='1';post1.submit();}else{return false;};" onMouseOver="overTD(this,'#d9dbdd');" onMouseOut="outTD(this,'');"><a href="Javascript:"><!--<img src='image/exit.png'><br>Salir</a></td>-->
		</tr>
	</table>
	<hr>
<?php
...
#				$area_etiqueta="Registro de Profesional:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ";
				$area_etiqueta=" ";
			}else{
				$area_etiqueta="Solicita registro como:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ";
			};
?>
		<input type="hidden" name="ciu_co" value="<?php if($stp['Ciudadr_Paisr_idPaisr']!=''){echo $stp['Ciudadr_Paisr_idPaisr'];};?>">
...
echo '<td><a href="JavaScript:" onClick="cher.cod.value=\''.$get_cod['RegistroPCodigo'].'\'; cher.target=\'pr\';cher.action=\'calificarServicio.php\';cher.submit();"> <img src="image/calificarServicio.png"></a> </td>';			
?>
			</tr>
...
<!--			<td width='1%'>
				...
				<input type='hidden' name='idFa<?php echo $cont;?>' value='<?php echo $row['idFAcademica']?>'>
					<div id='dnit<?php echo $cont;?>' style="DISPLAY:none; ">
							<SELECT NAME="NivelT<?php echo $cont;?>" size="1"  onFocus="set_bgcolor(this,'#85A3BB')" onBlur="set_bgcolor(this,'')">
								<OPTION <?php if($row['FAcademicaNivel']==0){ echo 'selected';}; ?> value="0">Nivel T&eacute;cnico o Tecnol&oacute;gico Superior</OPTION>
			...
			<td><b>NOMBRE T&Iacute;TULO:</b><br><?php echo str_replace('especificar','',$row['FAcademicaNTitulo'])?><br>
				<div id='dnti<?php echo $cont;?>' style="DISPLAY:none; ">
					<input type="text" name="ntitulo<?php echo $cont;?>" value="<?php echo $row['FAcademicaNTitulo']?>" size="20" onFocus="set_bgcolor(this,'<?php echo $usr_bgc?>')" onBlur="set_bgcolor(this,'')">
				</div><script>expandit2('dnti<?php echo $cont;?>',0)</script>
			</td>
			<td><b>INSTITUCI&Oacute;N:</b><br><?php echo str_replace('especificar','',$row['FAcademicaInstitucion'])?><br>
				<div id='dnin<?php echo $cont;?>' style="DISPLAY:none; ">
					<input type="text" name="ninstitucion<?php echo $cont;?>" value="<?php echo $row['FAcademicaInstitucion']?>" size="20" onFocus="set_bgcolor(this,'<?php echo $usr_bgc?>')" onBlur="set_bgcolor(this,'')">
				</div><script>expandit2('dnin<?php echo $cont;?>',0)</script>
			</td>
			<td><b>FECHA GRADUACI&Oacute;N:</b><br><?php echo str_replace('-','/',str_replace('0000-00-00','',$row['FAcademicaFecGrado']))?><br>
				<div id='dfti<?php echo $cont;?>' style="DISPLAY:none; ">
					<input type="text" name="ftitulo<?php echo $cont;?>" value="<?php echo str_replace('-','/',$row['FAcademicaFecGrado'])?>" size="10" maxlength="10" onFocus="set_bgcolor(this,'<?php echo $usr_bgc?>')" onBlur="set_bgcolor(this,'')" >
				</div><script>expandit2('dfti<?php echo $cont;?>',0)</script>
			</td>
			<td><b>N&Uacute;MERO DE REGISTRO EN SENESCYT:</b><br><?php echo eregi_replace("[1-9]{1}-actualice-".$stp['idProfesional'],'',$row['FAcademicaCSenescyt'])?><br>
				<div id='dcod<?php echo $cont;?>' style="DISPLAY:none; ">
					<input type="text" name="codigo<?php echo $cont;?>" value="<?php echo $row['FAcademicaCSenescyt']?>" size="20" maxlength="45" onFocus="set_bgcolor(this,'<?php echo $usr_bgc?>')" onBlur="set_bgcolor(this,'')" >
...
?>
		<TR bgcolor='#ffffff'>
			<td colspan='6' align='center'>
				<div id='addtit' style="DISPLAY: none;"><input class='buton' type="button" name="addti" value="Agregar T&iacute;tulo(s)" onClick="abrir('','add_ti',ancho*0.9,alto*0.75,0);post1.target='add_ti';post1.action='post-stat-agrega-titulo.php';post1.submit();post1.target='_self';return false;"></div><script>expandit2('addtit',0)</script>
			</td>
		</tr>
	</table><br>
<?php
...
?>
		...
		<div id='edit' style="DISPLAY: none; "> 
			<input class='buton' type="button" name="send" value="Actualizar y Repostular" onClick="if(validate()){post1.target='_self';post1.action='post2-edit.php';post1.submit();}">
			<input class='buton' type="button" name="cancel" value="Cancelar cambios"  onClick="post1.reset();_edit(0);resiz('<?php echo $_POST['ifid']?>')">
		</div>
   
    <script>expandit2('edit',0);
<?php
     if ($_POST['gen_new_post']==1){
      echo "expandit2('edit',1);";
      };
?>//ese uno ponel el disply=none a display:; en el div</script>
	</center>
<?php
	};
?>
...
<?php
	$old_posts = $db->query("SELECT * FROM Postulacion WHERE Profesiones_idProfesiones ='".$_POST['posts']."' and PostulacionFechaF is not NULL");
	if($old_posts->rowCount())
	echo "<center><H1> SOLICITUDES DE REGISTRO ANTERIORES</H1></center>";
		while($o_posts = $old_posts->fetch(PDO::FETCH_ASSOC)) {
?>
				<table align='center' border='0' cellspacing='1' cellpadding='2' class='seccion'>
					<tr align='center'>
						<td><b>Fecha Postulaci&oacute;n:</b></td>
						<td><b>Estado:</b></td>
						<!--<td><b>Proceso de Validaci&oacute;n:</b></td>
					...
					<tr bgcolor='#fffff' align='center'>
						<td><?php echo $o_posts['PostulacionFechaI']?></td>
						<td> <?php echo format_cont_db('PostulacionEstado',$o_posts['PostulacionEstado'])?></td>
						<!--<td>
<?php
							if(!$stpos['PostulacionVerificado']){
								echo format_cont_db('PostulacionAsignado',$stpos['PostulacionAsignado']);
...
?>
						...
						<td> <?php echo format_cont_db('PostulacionVerificado',$stpos['PostulacionVerificado'])?></td>-->
<?php
			($o_posts['PostulacionAprobado']=='aprobada')?$bg='#b0e6a7':$bg='#f1a09c';
?>
						<td bgcolor='<?php echo $bg?>'> <?php echo format_cont_db('PostulacionAprobado',$o_posts['PostulacionAprobado'])?></td>
						<td> <?php echo $o_posts['PostulacionFechaF']?></td>
					</tr>
				</table>
<?php
		};
