	...
	if ($stmt->rowCount()>0){
		$us= $stmt->fetch(PDO::FETCH_ASSOC);
			if (isset($token) and !$us['userEstado'] and $us['userTokenReg']==$token){
				$sql_act_user="UPDATE user SET userTokenReg=NULL,password=password('".$npass."'),userEstado=1 WHERE username='".$user."'";//activo la cuenta de usuario
				try {
				  $db->beginTransaction();
...
?>
			<script>alert('Cambio de clave exitoso, ingrese con su nueva clave para actualizar su datos precargados y habilitar su registro en la base de datos luego del proceso de revision por parte del INPC');
