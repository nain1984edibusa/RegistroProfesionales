...
//			alert(cont.charCodeAt(cu+1));
			flag=true;
			alert('Para evitar problemas de visualizacion de caracteres latinos \n Caracteres no permitidos -> '+cont.charAt(cu)+' <- No ingrese vocales tildadas ni letra enie');
			return flag;
		  }
		};
	};
...
/*		if ((j==0 && document.forms[0].valor.value.length == 0 ))
	...
	function check_mail(message,cont,lerr)
	{
		if ( cont.length > 0 )
		{
			...
			{
				alert ('Verifique el '+message+', no puede contener ni espacios ni caracteres especiales a parte de (_)');
				lerr=true;
			};
		};
		return lerr;
...
/*		if ((cu==0 && document.forms[0].formula.value.length == 0 ))
	...
	function check_ci(cont,lerr,isci)
	{
		...
		{
			flag=true;
			ver+="\nAlgun dígito ha sido mal ingresado";
		};
		if (flag)
		{
